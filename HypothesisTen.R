...
correlationsPheno17<- flattenCorrMatrix(phenogMatrix_17$r,phenogMatrix_17$P)

phenogMatrix_18<- dat18 %>% 
  tidylog::select(-rn)
phenogMatrix_18<- rcorr(as.matrix(phenogMatrix_18))

correlationsPheno18<- flattenCorrMatrix(phenogMatrix_18$r,phenogMatrix_18$P)
...
plot(as.phylo(hClustering_17), hang = -1, no.margin = T)
...
clus18 = cutree(hClustering_18, 3)
...
pvClust_17<- pvclust(as.matrix(dat17[,2:ncol(dat17)]), 
                     method.hclust="ward.D2", 
                     method.dist="abscor", 
                     use.cor="pairwise.complete.obs", 
                     nboot=10000)
plot(pvClust_17)
pvrect(pvClust_17,alpha = 0.95)
seplot(pvClust_17)

pvClust_18<- pvclust(as.matrix(dat18[,2:ncol(dat18)]),
                     method.hclust="ward.D2", 
                     method.dist="abscor", 
                     use.cor="pairwise.complete.obs", 
                     nboot=10000)
pvrect(pvClust_18,alpha = 0.95)
seplot(pvClust_18)
