...
FROM wheatgenetics.phenotype LEFT JOIN wheatgenetics.plot ON 
...
#save original plot information 
saveRDS(pheno, 
...
pheno_Wide$NormGRWT<- (pheno_Wide$GRWT *13) / pheno_Wide$MOIST


## Addition of HTP traits for 2018 data

htpPheno<- c("GNDVI","GRVI","height","NDRE","NDVI","Nir","RE")

htpFileLoad<- function(htp, f, ...) {
  for (i in htp) {
    fileNames<- list.files(path = "/Users/megzcalvert/Dropbox/Research_Poland_Lab/AM Panel/Phenotype_Database/2018_Ashland_AM3_traits_UAS",
                           full.names = T,
                           pattern = paste0("_",i))
...
pheno<- htpFileLoad(htpPheno,pheno_Wide)
...
correlations<- function(x, ...) {
  ...
  ggcorr(corDa, label_size = 1)
}

correlations(pheno)

ggpairs(pheno[,c(8:13,15)])
...
rm(all.opts,blues,chipLines,chrSum,cmg_dot,cor,cordist,cpg_dot,
   cpm_dot,geno,geno.opts,geno.optsDesc,genoOb,her2017,Her2017,
   lineInfo,misc.opts,misc.optsDesc,missingGenotype,missingPhenotype,
   opts,opts.desc,output,pcaAM,pd,pheno,pheno.opts,pheno.optsDesc,
   phenoDat,phenoLines,phenoOb,plotopts,plotopts.optsDesc,program,res2,
...
gwaBlueHddt <- rrBLUP::GWAS(pheno = blues, 
                            geno = snpChip, 
                            fixed = "hday", 
                            P3D = F, 
                            plot = F)
...
write.table(gwaBlue, 
            file = "~/Dropbox/Research_Poland_Lab/AM Panel/R/rrBlup/gwaBLUES18.txt", sep = "\t",
...
ggbiplot(b28.pca, groups = b.program)
