...
pheno17 <- pheno %>%
  filter(year == "17") %>%
  tidylog::select(
    entity_id, trait_id, phenotype_value, phenotype_date, Variety,
...
write.table(pheno17, "./Phenotype_Database/IndicatorPCTHEAD_17.txt",
  quote = F,
  sep = "\t", na = "NA", row.names = F, col.names = T
)
write.table(pheno18, "./Phenotype_Database/IndicatorPCTHEAD_18.txt",
  quote = F,
  sep = "\t", na = "NA", row.names = F, col.names = T
)
...
htp17long <- htp17long %>%
  dplyr::rename(
    entity_id = Plot_ID, trait_id = ID, phenotype_value = value,
    phenotype_date = Date
  ) %>%
...
htpPct17 <- htp17long %>%
  ...
  tidylog::select(
    entity_id, Variety, phenotype_date, trait_id, phenotype_value,
...
rm(
  hddt17, hddt18, htp17, htp18Wide, htp17long, htp18long, pheno, pheno_long, pheno17,
  pheno18, plotData17, plotData18, htpPheno, path, htpFileLoad
)

###############################################################################
write.table(htpPct17, "./Phenotype_Database/htpPct17.txt",
  quote = F,
  sep = "\t", na = "NA", row.names = F, col.names = T
)
write.table(htpPct18, "./Phenotype_Database/htpPct18.txt",
  quote = F,
  sep = "\t", na = "NA", row.names = F, col.names = T
)
...
fit18Train <- train(Ind ~ GNDVI + GRVI + NDRE + NDVI,
  data = htpPct18_wide,
