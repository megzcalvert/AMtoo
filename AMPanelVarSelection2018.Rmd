...
pheno_long<- subset(pheno_long, 
                    pheno_long$trait_id != "AWNS" )

pheno_pcthead<- subset(pheno_long, 
                       pheno_long$trait_id == "PCTHEAD" )
...
pheno_pcthead = 
  pheno_pcthead[order(pheno_pcthead$entity_id, pheno_pcthead$phenotype_date), ]  ## sort the rows by plot and date
...
test = runLogReg(dates=sample$phenotype_date, 
...
pheno_Wide <- mutate(
  dcast(pheno_long,entity_id + year + Variety + rep + 
...
pheno_Wide <- left_join(pheno_Wide, plotDates, 
                        by = c("entity_id" = "plots"))

histFacet.plot <- function(x, results, ...) {
  
  md<- names(x) %in% c("rn","Taxa","year","rep",
                       "block","column","range", "entity_id")
  traits <- names(x[ , !md])
  par(mfrow=c(35,3))
  for (i in traits) {
...
rm(awns, data, gndvi,grvi,hHTP,iniNames,modNames,ndre,ndvi,
   nir,pheno_long,pheno_Wide,redE,vis.logistic,test,sample,
   hddt,head.dates,n,pheno_pcthead,plotDates,i,plots,test.plot)
...
chipLines<- as.data.frame(unique(
  colnames(snpChip[, 13:ncol(snpChip)])
))
...
pheno<- as.data.frame(semi_join(pheno, chipLines, 
                                by = c("Taxa" = "snpChip")))
...
calcH2r <- function(dat, fill = NA, ...) {
  ...
  effectvars <- names(dat) %in% c("block","rep","Taxa", 
                                  "year","column","row","experiment_id")
...
pheno <- as.data.frame( pheno[, !names(pheno) %in%
                                c("X20171205_GNDVI","X20171120_GRVI",
                                  ...
                                  "X20180529_NDVI","X20171120_Nir",                                              "X20171127_Nir","X20171205_Nir",
                                  "X20171215_Nir","X20180529_Nir",
                                  "X20171120_RE","X20171127_RE",                                                 "X20171205_RE","X20171215_RE",
                                  "X20180529_RE","X20171127_height")])

blues.rb <- function(traits, dat = ".") {
  b<- as.data.frame(fixef(lmer(
    paste0(traits, "~ 0 + Taxa + (1|rep) + (1|rep:block)"), 
    data = dat)))
}
...
effectvars <- names(pheno) %in% 
  c("block", "rep", "Taxa", "year", 
    "column", "row", "experiment_id", "HDDT")
...
library(MASS)
bluesT<- as.data.frame(blues[, !names(blues) %in% 
                               c("Taxa")])
...
while(any(all_vifs > 10)){
  ...
  myFormBVIF <- as.formula(
    paste("GRWT ~ ", paste (signif_all, collapse=" + "), sep="")
    )  # new formula
...
selectedFitBlue<-lm(myFormBVIF, data = bluesT)
...
while(length(not_significant) > 0){
  ...
  myFormBVS <- as.formula(
    paste("GRWT ~ ", paste (all_vars, collapse=" + "), sep="")
    )  # new formula
...
bluPred<- data.matrix(blues[, !names(blues) %in% 
                              c("GRWT","Taxa")])
bluResp<- data.matrix(blues[, names(blues) %in% 
                              c("GRWT")])

lassBlu <- glmnet(bluPred,bluResp,alpha = 1)

plot(lassBlu,xvar = "lambda", 
     label = T,
     cex=2, 
     cex.lab=1.5, 
     cex.axis=1.5)
...
X20171127_NDRE, X20180404_NDRE, X20171218_GRVI, X20180412_NDVI, X20180514_GNDVI, X20180419_NDRE, X20180606_NDRE, X20171215_GRVI, X20180504_NDRE, X20171127_GRVI, X20180516_GNDVI, X20171218_height, X20180423_GNDVI, X20180529_NDRE, X20171218_NDRE

```{r LassoBluRes, fig.height=13, fig.width=13, echo=FALSE, message=FALSE, warning=FALSE}
fitLassoB<- lm(
  GRWT ~ X20171127_NDRE + X20180404_NDRE + X20171218_GRVI + 
    X20180412_NDVI + X20180514_GNDVI + X20180419_NDRE + 
    X20180606_NDRE + X20171215_GRVI + X20180504_NDRE + 
    X20171127_GRVI + X20180516_GNDVI + X20171218_height + 
    X20180423_GNDVI + X20180529_NDRE + X20171218_NDRE, 
  data = blues)
summary(fitLassoB)
...
coef(cvLasBlu)
cvLasBluL<- glmnet(bluPred,bluResp, 
                   lambda = cvLasBlu$lambda.min, 
                   alpha = 1)
...
fitLassoB<- lm(GRWT ~ MOIST + PTHT + SPNAREA + hday + 
                 X20171215_GNDVI + X20180514_GNDVI + X20180504_GRVI + 
                 X20180529_GRVI + X20171120_height + X20171215_height + 
                 X20180423_height + X20180504_height + X20180514_height + 
                 X20180516_height + X20180606_height + X20180613_height + 
                 X20180529_NDRE + X20180613_NDRE + X20171120_NDVI + 
                 X20180516_NDVI + X20180606_NDVI + X20180613_NDVI + 
                 X20180423_Nir + X20180514_Nir + X20180613_Nir + X20171218_RE,
               data = pheno)
summary(fitLassoB)
...
# Corr Model
bluCorrLoCV <- train(GRWT ~ X20180514_GNDVI + X20180516_GRVI + 
                       X20180514_NDRE + X20180516_NDRE +  
                       X20180514_NDVI + X20180516_NDVI, 
                     data = blues, 
                     method = "lm",
                     trControl = train.control)
...
# Her*Corr Model
bluCorrHerLoCV <- train(GRWT ~ hday + PTHT + SPNAREA + 
                          X20180419_RE + X20180423_RE + X20180504_height + 
                          X20180504_Nir + X20180504_RE + X20180514_height + 
                          X20180514_NDVI + X20180516_height + 
                          X20180529_height + X20180606_GNDVI + 
                          X20180606_GRVI + X20180606_NDRE + 
                          X20180606_NDVI + X20180606_Nir + X20180613_GNDVI + 
                          X20180613_GRVI + X20180613_height + 
                          X20180613_NDRE + X20180613_NDVI + 
                          X20180613_Nir + X20180613_RE, 
                        data = blues, 
                        method = "lm",
                        trControl = train.control)
# Summarize Full Model
...
# Back Selection 
bluBSLoCV <- train(GRWT ~ MOIST + PTHT + hday + X20171120_GNDVI + 
                     X20180423_GNDVI + X20180514_GNDVI + 
                     X20180516_GNDVI + X20180606_GNDVI + X20180613_GNDVI +
                     X20171127_GRVI + X20171218_GRVI + X20180412_GRVI + 
                     X20180419_GRVI + X20180423_GRVI + 
                     X20180504_GRVI + X20180516_GRVI + 
                     X20180606_GRVI + X20171218_height + 
                     X20180423_height + X20180613_height + 
                     X20171218_NDRE + X20180404_NDRE + X20180419_NDRE + 
                     X20180423_NDRE + X20180504_NDRE + X20180514_NDRE + 
                     X20180529_NDRE + X20180606_NDRE + X20180613_NDRE + 
                     X20180412_NDVI + X20180419_NDVI + X20180606_NDVI + 
                     X20180613_NDVI + X20180404_Nir + X20180423_Nir +
                     X20180516_Nir + X20180606_Nir + X20171218_RE + 
                     X20180404_RE + X20180423_RE + X20180504_RE + 
                     X20180514_RE + X20180606_RE + X20180613_RE, 
                   data = bluesT, 
                   method = "lm",
                   trControl = train.control)
...
# Back Selection VIF
bluBSVLoCV <- train(GRWT ~ MOIST + PTHT + hday + X20171120_GNDVI +
                      X20180514_GNDVI + X20180613_GNDVI + X20171127_GRVI + 
                      X20171218_GRVI + X20180504_GRVI + X20180516_GRVI +
                      X20171218_height + X20180423_height + X20180613_height + 
                      X20180529_NDRE + X20180606_NDVI + X20180613_NDVI +
                      X20180404_Nir + X20180423_Nir + X20180516_Nir + 
                      X20180606_Nir + X20171218_RE + X20180404_RE + 
                      X20180423_RE + X20180504_RE + X20180514_RE + 
                      X20180606_RE + X20180613_RE, 
                    data = bluesT, 
                    method = "lm",
                    trControl = train.control)
# Summarize Full Model
...
bluBSVSLoCV <- train(GRWT ~ MOIST + PTHT + hday + 
                       X20180514_GNDVI + X20171218_height + 
                       X20180613_height + X20180529_NDRE + X20180606_NDVI + 
                       X20171218_RE, 
                     data = bluesT, 
                     method = "lm",
                     trControl = train.control)
...
# LASSO 
bluLassLoCV <- train(GRWT ~ X20171127_NDRE + X20180404_NDRE + 
                       X20171218_GRVI + X20180412_NDVI + X20180514_GNDVI + 
                       X20180419_NDRE + X20180606_NDRE + X20171215_GRVI + 
                       X20180504_NDRE + X20171127_GRVI + X20180516_GNDVI + 
                       X20171218_height + X20180423_GNDVI + X20180529_NDRE + 
                       X20171218_NDRE, 
                     data = bluesT, 
                     method = "lm",
                     trControl = train.control)
...
bluLasLcvLoCV <- train(GRWT ~ MOIST + hday + X20180514_GNDVI + 
                         X20180516_GNDVI + X20171120_height + 
                         X20180529_NDRE + X20180516_NDVI + 
                         X20180606_NDVI + X20171218_RE, 
                       data = bluesT, 
                       method = "lm",
                       trControl = train.control)
...
bluLasLcvLoCV <- train(GRWT ~ MOIST + PTHT + SPNAREA + hday + 
                         X20171215_GNDVI + X20180514_GNDVI + 
                         X20180504_GRVI + X20180529_GRVI + X20171120_height + 
                         X20171215_height + X20180423_height + 
                         X20180504_height + X20180514_height + 
                         X20180516_height + X20180606_height + 
                         X20180613_height + X20180529_NDRE + 
                         X20180613_NDRE + X20171120_NDVI + 
                         X20180516_NDVI + X20180606_NDVI + 
                         X20180613_NDVI + X20180423_Nir + 
                         X20180514_Nir + X20180613_Nir + X20171218_RE, 
                       data = bluesT, 
                       method = "lm",
                       trControl = train.control)
...
# Corr Model
bluCorrkCV <- train(GRWT ~ X20180514_GNDVI + X20180516_GRVI + 
                      X20180514_NDRE + X20180516_NDRE +  
                      X20180514_NDVI + X20180516_NDVI, 
                    data = blues, 
                    method = "lm",
...
# Her*Corr Model
bluCorrHerkCV <- train(GRWT ~ hday + PTHT + SPNAREA + X20180419_RE +
                         X20180423_RE + X20180504_height + X20180504_Nir +
                         X20180504_RE + X20180514_height + X20180514_NDVI +
                         X20180516_height + X20180529_height + 
                         X20180606_GNDVI + X20180606_GRVI + 
                         X20180606_NDRE + X20180606_NDVI + 
                         X20180606_Nir + X20180613_GNDVI + 
                         X20180613_GRVI + X20180613_height + 
                         X20180613_NDRE + X20180613_NDVI + 
                         X20180613_Nir + X20180613_RE, 
                       data = blues, 
...
# Back Selection 
bluBSkCV <- train(GRWT ~ MOIST + PTHT + hday + X20171120_GNDVI +
                    X20180423_GNDVI + X20180514_GNDVI + 
                    X20180516_GNDVI + X20180606_GNDVI + X20180613_GNDVI + 
                    X20171127_GRVI + X20171218_GRVI + X20180412_GRVI + 
                    X20180419_GRVI + X20180423_GRVI + X20180504_GRVI +
                    X20180516_GRVI + X20180606_GRVI + 
                    X20171218_height + X20180423_height + X20180613_height + 
                    X20171218_NDRE + X20180404_NDRE + 
                    X20180419_NDRE + X20180423_NDRE + 
                    X20180504_NDRE + X20180514_NDRE + 
                    X20180529_NDRE + X20180606_NDRE + 
                    X20180613_NDRE + X20180412_NDVI + 
                    X20180419_NDVI + X20180606_NDVI + 
                    X20180613_NDVI + X20180404_Nir + 
                    X20180423_Nir + X20180516_Nir + 
                    X20180606_Nir + X20171218_RE + 
                    X20180404_RE + X20180423_RE + 
                    X20180504_RE + X20180514_RE + 
                    X20180606_RE + X20180613_RE, 
                  data = bluesT, 
                  method = "lm",
                  trControl = train.control)
...
# Back Selection VIF
bluBSVkCV <- train(GRWT ~ MOIST + PTHT + hday + 
                     X20171120_GNDVI + X20180514_GNDVI + 
                     X20180613_GNDVI + X20171127_GRVI + 
                     X20171218_GRVI + X20180504_GRVI + 
                     X20180516_GRVI + X20171218_height + 
                     X20180423_height + X20180613_height + 
                     X20180529_NDRE + X20180606_NDVI + 
                     X20180613_NDVI + X20180404_Nir + 
                     X20180423_Nir + X20180516_Nir + 
                     X20180606_Nir + X20171218_RE + 
                     X20180404_RE + X20180423_RE + X20180504_RE + 
                     X20180514_RE + X20180606_RE + 
                     X20180613_RE, 
                   data = bluesT, 
                   method = "lm",
                   trControl = train.control)
...
# Back Selection VIF and significance
bluBSVSkCV <- train(GRWT ~ MOIST + PTHT + hday + 
                      X20180514_GNDVI + X20171218_height + 
                      X20180613_height + X20180529_NDRE + 
                      X20180606_NDVI + X20171218_RE, 
                    data = bluesT, 
...
# LASSO 
bluLasskCV <- train(GRWT ~ X20171127_NDRE + X20180404_NDRE + 
                      X20171218_GRVI + X20180412_NDVI + 
                      X20180514_GNDVI + X20180419_NDRE + 
                      X20180606_NDRE + X20171215_GRVI + 
                      X20180504_NDRE + X20171127_GRVI + 
                      X20180516_GNDVI + X20171218_height + 
                      X20180423_GNDVI + X20180529_NDRE + 
                      X20171218_NDRE, 
                    data = bluesT, 
                    method = "lm",
                    trControl = train.control)
...
# LASSO minimised lambda cv
bluLasLcvkCV <- train(GRWT ~ MOIST + hday + X20180514_GNDVI + 
                        X20180516_GNDVI + X20171120_height + 
                        X20180529_NDRE + X20180516_NDVI + 
                        X20180606_NDVI + X20171218_RE, 
                      data = bluesT, 
...
# LASSO minimised lambda 
bluLasLcvkCV <- train(GRWT ~ MOIST + PTHT + SPNAREA + hday + 
                        X20171215_GNDVI + X20180514_GNDVI + 
                        X20180504_GRVI + X20180529_GRVI + 
                        X20171120_height + X20171215_height + 
                        X20180423_height + X20180504_height + 
                        X20180514_height + X20180516_height + 
                        X20180606_height + X20180613_height + 
                        X20180529_NDRE + X20180613_NDRE + 
                        X20171120_NDVI + X20180516_NDVI + 
                        X20180606_NDVI + X20180613_NDVI + 
                        X20180423_Nir + X20180514_Nir + 
                        X20180613_Nir + X20171218_RE, 
                      data = bluesT,
                      method = "lm",
                      trControl = train.control)
# Summarize Full Model
...
# Corr Model
bluCorrkCV <- lm(GRWT ~ X20180514_GNDVI + X20180516_GRVI + 
                   X20180514_NDRE + X20180516_NDRE +  
                   X20180514_NDVI + X20180516_NDVI, 
                 data = blues)
# Summarize Corr Model
...
# Her*Corr Model
bluCorrHerkCV <- lm(GRWT ~ hday + PTHT + SPNAREA + X20180419_RE + 
                      X20180423_RE + X20180504_height + X20180504_Nir +
                      X20180504_RE + X20180514_height + X20180514_NDVI +
                      X20180516_height + X20180529_height + 
                      X20180606_GNDVI + X20180606_GRVI +
                      X20180606_NDRE + X20180606_NDVI + X20180606_Nir +
                      X20180613_GNDVI + X20180613_GRVI + X20180613_height +
                      X20180613_NDRE + X20180613_NDVI + 
                      X20180613_Nir + X20180613_RE, 
                    data = blues)
# Summarize Full Model
...
# Back Selection VIF and significance
bluBSVSkCV <- lm(GRWT ~ MOIST + PTHT + hday + 
                   X20180514_GNDVI + X20171218_height + 
                   X20180613_height + X20180529_NDRE + 
                   X20180606_NDVI + X20171218_RE, 
                 data = bluesT)
...
# LASSO 
bluLasskCV <- lm(GRWT ~ X20171127_NDRE + X20180404_NDRE + 
                   X20171218_GRVI + X20180412_NDVI + 
                   X20180514_GNDVI + X20180419_NDRE + 
                   X20180606_NDRE + X20171215_GRVI + 
                   X20180504_NDRE + X20171127_GRVI + 
...
# LASSO minimised lambda cv
bluLasLcvkCV <- lm(GRWT ~ MOIST + hday + X20180514_GNDVI + 
                     X20180516_GNDVI + X20171120_height + 
                     X20180529_NDRE + X20180516_NDVI + 
                     X20180606_NDVI + X20171218_RE, 
                   data = bluesT)
...
# LASSO minimised lambda 
bluLasLcvkCV <- lm(GRWT ~ MOIST + PTHT + SPNAREA + hday +
                     X20171215_GNDVI + X20180514_GNDVI + 
                     X20180504_GRVI + X20180529_GRVI + 
                     X20171120_height + X20171215_height +
                     X20180423_height + X20180504_height + 
...
modelSum<- anova(blukCV,bluCorrkCV,bluCorrHerkCV,bluBSVSkCV,
                 bluLasskCV,bluLasLcvkCV)
modelSum
summary(modelSum)
```
