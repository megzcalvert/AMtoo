---
title: "AM Panel Variable Selection 2018"
...
Data will be pulled directly from SQL server, converted from long format to wide format. The HDDT is calculated via Jesse's regression based approach. High-throughput phenotypes are added. Outliers are removed, before broad-sense heritability and Best Linear Unbiased Estimators are calculated. Normality will be assessed before and after. Various model selection methods will then be assessed
...
pheno_long$year<- ff(pheno_long$entity_id, c("18ASH"), c("18"),
                     "NA", ignore.case = TRUE)

modNames<- tabyl(pheno_long$Variety)
names(modNames)[1:2] <- c("Variety", "Count")
knitr::kable(tabyl(pheno_long$trait_id), digits = 4, caption = "Number of observations per trait",table.attr = "style='width:50%;'")
knitr::kable(tabyl(pheno_long$rep), digits = 4, caption = "Number of observations of reps",table.attr = "style='width:50%;'")
knitr::kable(tabyl(pheno_long$year), digits = 4, caption = "Number of observations per year",table.attr = "style='width:50%;'")
...
knitr::kable(tabyl(pheno_long$trait_id), digits = 4, caption = "Final number of observations per traits", table.attr = "style='width:50%;'")
knitr::kable(tabyl(pheno_long$rep), digits = 4, caption = "Final number of observations per rep", table.attr = "style='width:50%;'")
knitr::kable(tabyl(pheno_long$year), digits = 4, caption = "Final number of observations per year", table.attr = "style='width:50%;'")
...
awns<- pheno_long[ , c("entity_id","awns")]
...
histFacet.plot <- function(x, results, ...) {
...
knitr::kable(tabyl(pheno$year), caption = "Number of observations per year", table.attr = "style='width:50%;'")
...
pheno <- as.data.frame( pheno[, !names(pheno) %in% c("X20171205_GNDVI","X20171120_GRVI",
...
write.table(blues, "~/Dropbox/Research_Poland_Lab/AM Panel/Phenotype_Database/cleanPheno18Blues.txt",
            quote = FALSE, row.names = F, col.names = T, sep = "\t", fileEncoding = "UTF-8")

rm(cmg_dot,cpg_dot,cpm_dot,pd,snpChip,myvars,awns,chipLines)

pheno<- as.data.frame( pheno[, !names(pheno) %in% c("NormGRWT","TESTWT","GRYLD","awns" )])
pheno<- na.omit(pheno)

blues<- as.data.frame(blues[, !names(blues) %in% c("NormGRWT", "TESTWT","GRYLD","awns")])
blues<- na.omit(blues)
...
#histFacet.plot(blues,'~/Dropbox/Research_Poland_Lab/AM Panel/Figures/Hist/','blues_2018')
```
...
plotlevel<-mvn(pheno[, !names(pheno) %in% c("Taxa","rep","block")], subset = NULL, 
...
knitr::kable(plotlevel$univariateNormality, digits = 4, caption = "Plot-level Normality tests",table.attr = "style='width:80%;'")

```

#### BLUES data normality tests
```{r NormalityBlues, echo=FALSE, message=TRUE, warning=FALSE}
bluelevel<-mvn(blues[, !names(blues) %in% c("Taxa")], subset = NULL, 
    mvnTest = c("mardia", "hz", "royston", "dh","energy"), 
    covariance = TRUE, 
    tol = 1e-25, 
    alpha = 0.5,
...
knitr::kable(bluelevel$univariateNormality, digits = 4, caption = "BLUE Normality tests", table.attr = "style='width:80%;'")

```

## Step-wises regression

Step-wise regression was performed with the MASS package in backward directions. 
...
selectedFitBlue$anova # display results
...
while(any(all_vifs > 10)){
  ...
  myFormBVIF <- as.formula(paste("GRWT ~ ", paste (signif_all, collapse=" + "), sep=""))  # new formula
...
while(length(not_significant) > 0){
  ...
  myFormBVS <- as.formula(paste("GRWT ~ ", paste (all_vars, collapse=" + "), sep=""))  # new formula
...
#### Step-wise Regression in both directions with control for Multicollinearity and variable significance
...
lassBlu$beta@Dimnames


```

Select phenotpyes BLUEs: 47,50,22,66,14,52,58,21,54,19,15,36,12,57,49
X20171127_NDRE   X20180404_NDRE   X20171218_GRVI   X20180412_NDVI   X20180514_GNDVI 
X20180419_NDRE   X20180606_NDRE   X20171215_GRVI   X20180504_NDRE   X20171127_GRVI  
X20180516_GNDVI  X20171218_height X20180423_GNDVI  X20180529_NDRE   X20171218_NDRE

```{r LassoBluRes, fig.height=13, fig.width=13, echo=FALSE, message=FALSE, warning=FALSE}
fitLassoB<- lm(GRWT ~ X20171127_NDRE + X20180404_NDRE + X20171218_GRVI + X20180412_NDVI + X20180514_GNDVI + X20180419_NDRE + X20180606_NDRE + X20171215_GRVI + X20180504_NDRE + X20171127_GRVI + X20180516_GNDVI + X20171218_height + X20180423_GNDVI + X20180529_NDRE + X20171218_NDRE, data = blues)
summary(fitLassoB)
...
bluPredS<- bluPred[,c(47,50,22,66,14,52,58,21,54,19,15,36,12,57,49)]
head(bluPredS)
...
cvLasBlu<- cv.glmnet(bluPred,bluResp, type.measure = "mse", nfolds = 10, alpha = 1)
plot(cvLasBlu)
cvLasBlu$lambda.min
cvLasBlu$lambda.1se
...
```{r LassoBluResL, fig.height=13, fig.width=13, echo=FALSE, message=FALSE, warning=FALSE}
fitLassoB<- lm(GRWT ~ MOIST + PTHT + SPNAREA + hday + X20171215_GNDVI + X20180514_GNDVI + X20180504_GRVI + X20180529_GRVI + X20171120_height + X20171215_height + X20180423_height + X20180504_height + X20180514_height + X20180516_height + X20180606_height + X20180613_height + X20180529_NDRE + X20180613_NDRE + X20171120_NDVI + X20180516_NDVI + X20180606_NDVI + X20180613_NDVI + X20180423_Nir + X20180514_Nir + X20180613_Nir + X20171218_RE, data = pheno)
summary(fitLassoB)
...
Leave one out cross-validation and K-fold cross-validation. These validations will also be performed on the Correlation models and Correlation*Heritability models. The caret package will be used to perform this validation
...
# Corr Model
bluCorrLoCV <- train(GRWT ~ X20180514_GNDVI + X20180516_GRVI + X20180514_NDRE + X20180516_NDRE +  X20180514_NDVI + X20180516_NDVI, data = blues, method = "lm",
                     trControl = train.control)
# Summarize Corr Model
print(bluCorrLoCV)

# Her*Corr Model
bluCorrHerLoCV <- train(GRWT ~ hday + PTHT + SPNAREA + X20180419_RE + X20180423_RE + X20180504_height + X20180504_Nir + X20180504_RE + X20180514_height + X20180514_NDVI + X20180516_height + X20180529_height + X20180606_GNDVI + X20180606_GRVI + X20180606_NDRE + X20180606_NDVI + X20180606_Nir + X20180613_GNDVI + X20180613_GRVI + X20180613_height + X20180613_NDRE + X20180613_NDVI + X20180613_Nir + X20180613_RE, data = blues, method = "lm",
                     trControl = train.control)
# Summarize Full Model
print(bluCorrHerLoCV)

# Back Selection 
bluBSLoCV <- train(GRWT ~ MOIST + PTHT + hday + X20171120_GNDVI + X20180423_GNDVI + 
    X20180514_GNDVI + X20180516_GNDVI + X20180606_GNDVI + X20180613_GNDVI + 
    X20171127_GRVI + X20171218_GRVI + X20180412_GRVI + X20180419_GRVI + 
    X20180423_GRVI + X20180504_GRVI + X20180516_GRVI + X20180606_GRVI + 
    X20171218_height + X20180423_height + X20180613_height + 
    X20171218_NDRE + X20180404_NDRE + X20180419_NDRE + X20180423_NDRE + 
    X20180504_NDRE + X20180514_NDRE + X20180529_NDRE + X20180606_NDRE + 
    X20180613_NDRE + X20180412_NDVI + X20180419_NDVI + X20180606_NDVI + 
    X20180613_NDVI + X20180404_Nir + X20180423_Nir + X20180516_Nir + 
    X20180606_Nir + X20171218_RE + X20180404_RE + X20180423_RE + 
    X20180504_RE + X20180514_RE + X20180606_RE + X20180613_RE, data = bluesT, method = "lm",
               trControl = train.control)
...
# Back Selection VIF
bluBSVLoCV <- train(GRWT ~ MOIST + PTHT + hday + X20171120_GNDVI + X20180514_GNDVI + 
    X20180613_GNDVI + X20171127_GRVI + X20171218_GRVI + X20180504_GRVI + 
    X20180516_GRVI + X20171218_height + X20180423_height + X20180613_height + 
    X20180529_NDRE + X20180606_NDVI + X20180613_NDVI + X20180404_Nir + 
    X20180423_Nir + X20180516_Nir + X20180606_Nir + X20171218_RE + 
    X20180404_RE + X20180423_RE + X20180504_RE + X20180514_RE + 
    X20180606_RE + X20180613_RE, data = bluesT, method = "lm",
               trControl = train.control)
# Summarize Full Model
print(bluBSVLoCV)

# Back Selection VIF and significance
bluBSVSLoCV <- train(GRWT ~ MOIST + PTHT + hday + X20180514_GNDVI + X20171218_height + 
    X20180613_height + X20180529_NDRE + X20180606_NDVI + X20171218_RE, data = bluesT, method = "lm",
               trControl = train.control)
# Summarize Full Model
print(bluBSVSLoCV)

# LASSO 
bluLassLoCV <- train(GRWT ~ X20171127_NDRE + X20180404_NDRE + X20171218_GRVI + X20180412_NDVI + X20180514_GNDVI + X20180419_NDRE + X20180606_NDRE + X20171215_GRVI + X20180504_NDRE + X20171127_GRVI + X20180516_GNDVI + X20171218_height + X20180423_GNDVI + X20180529_NDRE + X20171218_NDRE, 
...
bluLasLcvLoCV <- train(GRWT ~ MOIST + hday + X20180514_GNDVI + X20180516_GNDVI + X20171120_height + X20180529_NDRE + X20180516_NDVI + X20180606_NDVI + X20171218_RE, data = bluesT, method = "lm",
               trControl = train.control)
# Summarize Full Model
print(bluLasLcvLoCV)

# LASSO minimised lambda 
bluLasLcvLoCV <- train(GRWT ~ MOIST + PTHT + SPNAREA + hday + X20171215_GNDVI + X20180514_GNDVI + X20180504_GRVI + X20180529_GRVI + X20171120_height + X20171215_height + X20180423_height + X20180504_height + X20180514_height + X20180516_height + X20180606_height + X20180613_height + X20180529_NDRE + X20180613_NDRE + X20171120_NDVI + X20180516_NDVI + X20180606_NDVI + X20180613_NDVI + X20180423_Nir + X20180514_Nir + X20180613_Nir + X20171218_RE, data = bluesT, method = "lm",
               trControl = train.control)
# Summarize Full Model
print(bluLasLcvLoCV)
```
...
# Corr Model
bluCorrkCV <- train(GRWT ~ X20180514_GNDVI + X20180516_GRVI + X20180514_NDRE + X20180516_NDRE +  X20180514_NDVI + X20180516_NDVI, data = blues, method = "lm",
                     trControl = train.control)
# Summarize Corr Model
print(bluCorrkCV)

# Her*Corr Model
bluCorrHerkCV <- train(GRWT ~ hday + PTHT + SPNAREA + X20180419_RE + X20180423_RE + X20180504_height + X20180504_Nir + X20180504_RE + X20180514_height + X20180514_NDVI + X20180516_height + X20180529_height + X20180606_GNDVI + X20180606_GRVI + X20180606_NDRE + X20180606_NDVI + X20180606_Nir + X20180613_GNDVI + X20180613_GRVI + X20180613_height + X20180613_NDRE + X20180613_NDVI + X20180613_Nir + X20180613_RE, data = blues, method = "lm",
                     trControl = train.control)
# Summarize Full Model
print(bluCorrHerkCV)

# Back Selection 
bluBSkCV <- train(GRWT ~ MOIST + PTHT + hday + X20171120_GNDVI + X20180423_GNDVI + 
    X20180514_GNDVI + X20180516_GNDVI + X20180606_GNDVI + X20180613_GNDVI + 
    X20171127_GRVI + X20171218_GRVI + X20180412_GRVI + X20180419_GRVI + 
    X20180423_GRVI + X20180504_GRVI + X20180516_GRVI + X20180606_GRVI + 
    X20171218_height + X20180423_height + X20180613_height + 
...
# Back Selection VIF
bluBSVkCV <- train(GRWT ~ MOIST + PTHT + hday + X20171120_GNDVI + X20180514_GNDVI + 
    X20180613_GNDVI + X20171127_GRVI + X20171218_GRVI + X20180504_GRVI + 
    X20180516_GRVI + X20171218_height + X20180423_height + X20180613_height + 
    X20180529_NDRE + X20180606_NDVI + X20180613_NDVI + X20180404_Nir + 
    X20180423_Nir + X20180516_Nir + X20180606_Nir + X20171218_RE + 
...
# Back Selection VIF and significance
bluBSVSkCV <- train(GRWT ~ MOIST + PTHT + hday + X20180514_GNDVI + X20171218_height + 
    X20180613_height + X20180529_NDRE + X20180606_NDVI + X20171218_RE, data = bluesT, method = "lm",
               trControl = train.control)
# Summarize Full Model
print(bluBSVSkCV)
...
# LASSO 
bluLasskCV <- train(GRWT ~ X20171127_NDRE + X20180404_NDRE + X20171218_GRVI + X20180412_NDVI + X20180514_GNDVI + X20180419_NDRE + X20180606_NDRE + X20171215_GRVI + X20180504_NDRE + X20171127_GRVI + X20180516_GNDVI + X20171218_height + X20180423_GNDVI + X20180529_NDRE + X20171218_NDRE, 
                     data = bluesT, method = "lm",
               trControl = train.control)
# Summarize Full Model
print(bluLasskCV)
...
# LASSO minimised lambda cv
bluLasLcvkCV <- train(GRWT ~ MOIST + hday + X20180514_GNDVI + X20180516_GNDVI + X20171120_height + X20180529_NDRE + X20180516_NDVI + X20180606_NDVI + X20171218_RE, data = bluesT, method = "lm",
               trControl = train.control)
# Summarize Full Model
print(bluLasLcvkCV)

# LASSO minimised lambda 
bluLasLcvkCV <- train(GRWT ~ MOIST + PTHT + SPNAREA + hday + X20171215_GNDVI + X20180514_GNDVI + X20180504_GRVI + X20180529_GRVI + X20171120_height + X20171215_height + X20180423_height + X20180504_height + X20180514_height + X20180516_height + X20180606_height + X20180613_height + X20180529_NDRE + X20180613_NDRE + X20171120_NDVI + X20180516_NDVI + X20180606_NDVI + X20180613_NDVI + X20180423_Nir + X20180514_Nir + X20180613_Nir + X20171218_RE, data = bluesT, method = "lm",
               trControl = train.control)
# Summarize Full Model
print(bluLasLcvkCV)
```
...
#### BLUES level
```{r ModelSums, echo=TRUE, message=FALSE, warning=FALSE}

# Full Model 
blukCV <- lm(GRWT ~., data = blues)
# Summarize Full Model
summary(blukCV)

# Corr Model
bluCorrkCV <- lm(GRWT ~ X20180514_GNDVI + X20180516_GRVI + X20180514_NDRE + X20180516_NDRE +  X20180514_NDVI + X20180516_NDVI, data = blues)
# Summarize Corr Model
summary(bluCorrkCV)

# Her*Corr Model
bluCorrHerkCV <- lm(GRWT ~ hday + PTHT + SPNAREA + X20180419_RE + X20180423_RE + X20180504_height + X20180504_Nir + X20180504_RE + X20180514_height + X20180514_NDVI + X20180516_height + X20180529_height + X20180606_GNDVI + X20180606_GRVI + X20180606_NDRE + X20180606_NDVI + X20180606_Nir + X20180613_GNDVI + X20180613_GRVI + X20180613_height + X20180613_NDRE + X20180613_NDVI + X20180613_Nir + X20180613_RE, data = blues)
# Summarize Full Model
summary(bluCorrHerkCV)


# Back Selection VIF and significance
bluBSVSkCV <- lm(GRWT ~ MOIST + PTHT + hday + X20180514_GNDVI + X20171218_height + 
    X20180613_height + X20180529_NDRE + X20180606_NDVI + X20171218_RE, data = bluesT)
# Summarize Full Model
summary(bluBSVSkCV)

# LASSO 
bluLasskCV <- lm(GRWT ~ X20171127_NDRE + X20180404_NDRE + X20171218_GRVI + X20180412_NDVI + X20180514_GNDVI + X20180419_NDRE + X20180606_NDRE + X20171215_GRVI + X20180504_NDRE + X20171127_GRVI + X20180516_GNDVI + X20171218_height + X20180423_GNDVI + X20180529_NDRE + X20171218_NDRE, 
                     data = bluesT)
# Summarize Full Model
summary(bluLasskCV)

# LASSO minimised lambda cv
bluLasLcvkCV <- lm(GRWT ~ MOIST + hday + X20180514_GNDVI + X20180516_GNDVI + X20171120_height + X20180529_NDRE + X20180516_NDVI + X20180606_NDVI + X20171218_RE, data = bluesT)
# Summarize Full Model
summary(bluLasLcvkCV)

# LASSO minimised lambda 
bluLasLcvkCV <- lm(GRWT ~ MOIST + PTHT + SPNAREA + hday + X20171215_GNDVI + X20180514_GNDVI + X20180504_GRVI + X20180529_GRVI + X20171120_height + X20171215_height + X20180423_height + X20180504_height + X20180514_height + X20180516_height + X20180606_height + X20180613_height + X20180529_NDRE + X20180613_NDRE + X20171120_NDVI + X20180516_NDVI + X20180606_NDVI + X20180613_NDVI + X20180423_Nir + X20180514_Nir + X20180613_Nir + X20171218_RE, data = bluesT)
# Summarize Full Model
summary(bluLasLcvkCV)

modelSum<- anova(blukCV,bluCorrkCV,bluCorrHerkCV,bluBSVSkCV,bluLasskCV,bluLasLcvkCV)
modelSum
summary(modelSum)
```
